...
module.exports = {
  ...
  create_a_note: (req, res) => {
    ...
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate, private')
        ...
        var new_note = new Note({ name: mongoSanitize(body.name), body: mongoSanitize(body.body), type: mongoSanitize(body.type), user: mongoSanitize(result.user) });
    ...
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate, private')
        ...
        for (const key in req.body) {
          if (req.body.hasOwnProperty(key)) {
            sanitizedBody[key] = mongoSanitize(req.body[key]);
          }
        }
        Note.findOneAndUpdate({name: sanitizedNoteId }, sanitizedBody, { new: true }, function (err, note) {
          if (err) {
